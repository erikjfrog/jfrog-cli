...
def distributeToReleases(stage, version, rbcSpecName) {
    sh "jf rt rbc $stage-rb-$identifier $version --recursive=true --spec=${jfrogCliRepoDir}build/release_specs/$rbcSpecName --spec-vars=\"VERSION=$version;IDENTIFIER=$identifier\" --sign"
